function [xd,Wd,id,nd,WT,WP] = pwsdde_delay_interp(data,u,node_type,seg_ind)
%PWSDDE_DELAY_INTERP Evaluate delayed terms, corresponding segment indicies, 
%and interpolation coefficients in the multi point boundary value
%problem of PWS-DDE periodic orbits 
...
for i = 1:nt
    ...
    dtau(i,:) = feval(sys.tau,par,i,3);     % parameter Jacobians
    tau_type(i) = feval(sys.tau,par,i,1);   % delay types
end
ntau = ceil(max(tau)/period); % ntau*period is neccessary to cover tau_max
    ...
    for k = 1:nt
        ...
        for j = 1:Ntm
            ...
            if tau_type(k) == 1
                ...
                Wd{i,k}{j} = W;
                WP{i,k}(j,:,:) = -1/T(i0)*Wp*xbp{i0}*dtau(k,:);
                for n = 1:ntau+1
                ...
                Wd{i,k}{j} = 1/T(i0)*Wp;
                WP{i,k}(j,:,:) = -1/T(i0)^2*Wpp*xbp{i0}*dtau(k,:);
                for n=1:ntau+1
